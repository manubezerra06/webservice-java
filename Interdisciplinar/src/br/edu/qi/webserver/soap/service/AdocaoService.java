	...
	public RetornoAdocaoService adotarAnimal(AdocaoServiceDto adocaoServiceDto) {
		...
		for (Animal animal : newAdocao.getAnimais()) {
			...
			if (animal.getAdocao() != null) {
				...
				xmlModel.setErro("animal " + animal.getCodigo() + " jï¿½ estï¿½ adotado.");
				return xmlModel;
			}
			...
			if (count > 2) {
				RetornoAdocaoService xmlModel = new RetornoAdocaoService();
				xmlModel.setErro("sï¿½ pode adotar 2 animais de um mesmo tipo");
				return xmlModel;
			}
		...
		if (newAdocao.getInstituicao() == null) {
			RetornoAdocaoService xmlModel = new RetornoAdocaoService();
			xmlModel.setErro("instituiï¿½ï¿½o nï¿½o encontrada.");
			return xmlModel;
		}
		...
		if (newAdocao.getPessoa() == null) {
			...
			RetornoAdocaoService xmlModel = new RetornoAdocaoService();
			xmlModel.setErro("pessoa nï¿½o encontrada.");
			return xmlModel;
		}
